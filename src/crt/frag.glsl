...
void main() {
	...
	float r = texture2D(texture, uv_curve(uv) + vec2(wRcp*0.0), 0.0).r;
	float g = texture2D(texture, uv_curve(uv) + vec2(wRcp*3.0), 0.0).g;
	float b = texture2D(texture, uv_curve(uv) + vec2(wRcp*-3.0), 0.0).b;
