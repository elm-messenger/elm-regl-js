...
async function loadTexture(texture_url) {
    // Initialize textures
    image = new Image();
    ...
    // }
    await image.decode();
    image = regl.texture({ data: image, mag: 'linear', min: 'linear' });

}

async function main() {
    await loadTexture('build/a.png');
    const textJson = JSON.parse(readFileSync('build/a.json', 'utf8'));
    ...
    const prog = regl({
        ...
        attributes: {
            position: text.buffers.position,
            uv: text.buffers.uv
        },
        uniforms: {
            tMap: image,
        },
        elements: text.buffers.index,
        depth: { enable: false },
    })
